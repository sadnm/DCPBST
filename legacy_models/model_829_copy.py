...
class Encoder(nn.Module):
    def __init__(self, input_dim, latent_dim, dropout_p=0.2):
        ...
        # Define two hidden layer dimensions
        hidden_dim_1 = 1024
        hidden_dim_2 = 512

        # Build encoder network with two hidden layers
        self.encoder = nn.Sequential(
            # --- First hidden layer ---
            nn.Linear(input_dim, hidden_dim_1),
            nn.BatchNorm1d(hidden_dim_1), # Batch normalization
            nn.ReLU(),
            nn.Dropout(p=dropout_p),      # Dropout
            
            # --- Second hidden layer ---
            nn.Linear(hidden_dim_1, hidden_dim_2),
            nn.BatchNorm1d(hidden_dim_2), # Batch normalization
            nn.ReLU(),
            nn.Dropout(p=dropout_p)       # Dropout
        )
    ...
    def forward(self, x):
        ...
        Forward pass
        """
        # 1. Extract features through encoder network
        h = self.encoder(x)
        
        # 2. Compute mu and log_var from features
        mu = self.fc_mu(h)
        log_var = self.fc_log_var(h)
        
        # 3. Sample z via reparameterization trick
...
def katz_similarity(adjacency_matrix, beta):
    ...
    # Get maximum eigenvalue of adjacency matrix
...
def get_katz(adata):
    ...
    return pca(katz)# Output spatial feature map representing higher-order relations
...
class AvgReadout(nn.Module):
    ...
    def forward(self, seq, msk):
        if msk is None:
            ...
            # unsqueeze msk to [B, N, 1] for broadcasting
            msk = msk.unsqueeze(-1) 
            # Calculate sum for each graph in the batch
            sum_hidden = torch.sum(seq * msk, 1)
            # Calculate num of nodes for each graph, shape [B, 1]
...
class dgi(torch.nn.Module):
    ...
    def forward(self, pos_z, neg_z, msk=None):
        ...
        if msk is not None:
            # Ensure msk is float type for multiplication
            msk = msk.float()
            ...
            total_loss = pos_loss + neg_loss  # Binary cross-entropy loss, global summary based on original graph info
            
        return total_loss


class CrossAttention(nn.Module):  # Cross-attention for cross-modal alignment
...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        # A single linear layer generates Q, K, V simultaneously
    ...
    def forward(self, x, mask=None):
        ...
        mask (Tensor, optional): Mask tensor to ignore certain positions.
        """
        B, N, C = x.shape # B=batch size, N=sequence length, C=embedding dimension

        # 1. Linear projection and split into Q, K, V
        # qkv: [B, N, C*3] -> [B, N, 3, H, D] -> [3, B, H, N, D]
        qkv = self.qkv_proj(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2] # q, k, v each shape: [B, H, N, D]
        ...
        # 6. Concatenate multi-head results and pass through output projection
...
class GCN(nn.Module):
    def __init__(self, input_dim, output_dim, use_bias=True):
        ...
        adjacency = D^-1/2 A D^-1/2 is the normalized, standardized Laplacian matrix
        ...
        # Define GCN layer W weight shape
        self.weight = nn.Parameter(torch.Tensor(input_dim, output_dim))
        
        # Define GCN layer b weight matrix
        if self.use_bias:
    ...
    # Initialize W,b parameters in nn.Module class
    def reset_parameters(self):
    ...
    def forward(self,input_feature,adjacency):
        """Adjacency matrix is sparse, so use sparse matrix multiplication in computation
        ...
        support = torch.matmul(input_feature, self.weight)  # H * W
        output = torch.sparse.mm(adjacency, support)  # (D^-1/2 A D^-1/2) * (H * W)
...
def scipy_sparse_to_torch_sparse(sparse_mx):
    """Convert a scipy sparse matrix to a torch sparse tensor, ensuring float32 type."""
    ...
    # torch.sparse.FloatTensor creates a float32 sparse tensor
    return torch.sparse.FloatTensor(indices, values, shape)


def _build_symmetric_knn_graph(feature_matrix, n_neighbors):
    """
    An efficient helper function to build a symmetric k-NN adjacency matrix.
    """
    # 1. Efficiently compute k-nearest-neighbor graph using kneighbors_graph
    #    Returns a sparse matrix where value 1 indicates neighbor
    adj = kneighbors_graph(
        feature_matrix, 
        n_neighbors=n_neighbors, 
        mode='connectivity', 
        include_self=False  # Do not include node itself as neighbor
    )
    
    # 2. Convert adjacency matrix to symmetric
    #    If A is neighbor of B, then B should also be neighbor of A
    adj = adj + adj.T
...
def construct_interaction(adata, n_neighbors=6):
    """
    (Optimized version) Build spot-to-spot k-NN graphs for spatial, gene, and image features respectively.
    """
    ...
    try:
        # Try to get gene expression matrix, check if exists and non-empty
        gene_expression = adata.X
        if gene_expression is None:
            raise ValueError("adata.X is None (no gene expression matrix)")
        ...
        # Symmetrize
        adj_x = 0.5 * (adj_x + adj_x.T)
        adata.obsm['adj_spot_x'] = adj_x

    except AttributeError:
        print("X attribute not found in adata (no gene expression matrix), cannot build adj_spot_x")
    except ValueError as e:
        print(f"Gene expression matrix invalid: {e}")
    ...
    print("All adjacency matrices constructed.")

# Final multi-modal weighted fusion
...
class ModalRobustnessDiagnoser:
    """
    A class encapsulating modality robustness diagnosis and re-learning logic.

    This class is responsible for:
    1. Saving initial encoder weights.
    2. Applying specified augmentations to input data (Gaussian noise for images, Dropout for others).
    3. Diagnosing each modality by comparing clustering stability before and after augmentation.
    4. Performing "re-learning" update on encoder weights based on diagnosis results (alpha values).
    """
    def __init__(self, 
        ...
        Args:
            encoders (nn.ModuleList): List of encoders to be diagnosed and updated.
            data_types (list): A list of strings indicating the type of each data view,
                               e.g. ['gene', 'image'].
            device (str): Computing device.
            n_clusters (int): Number of K-means clusters used for diagnosis.
            noise_level (float): Gaussian noise intensity for images.
            dropout_rate (float): Dropout ratio for gene expression.
        """
        ...
        # Key: deep copy initial state of encoders
    ...
    def _augment_data(self, data: torch.Tensor, data_type: str) -> torch.Tensor:
        """Apply different augmentations based on data type."""
        if data_type == 'image':
            # Assuming image data needs to be reshaped to [B, C, H, W] format
            # Implementation here needs to be adjusted according to actual data dimensions
            # This is an example, assuming input is already a 2D or 3D image-like structure
            original_shape = data.shape
            if len(original_shape) == 2: # [Spots, Features] -> [Spots, 1, H, W]
                # Try to reshape features into a square image
                h = w = int(np.sqrt(original_shape[1]))
                if h * w == original_shape[1]:
                    data = data.reshape(original_shape[0], 1, h, w)
                else: # If not possible, just add noise directly
                    noise = torch.randn_like(data) * self.noise_level
                    return torch.clamp(data + noise, 0.0, data.max().item())

            noise = torch.randn_like(data) * self.noise_level
            noisy_data = torch.clamp(data + noise, 0.0, 1.0)  # Assuming image is normalized to [0,1]
            ...
            # Use dropout for unknown types by default
            mask = torch.bernoulli(torch.full_like(data, 1 - self.dropout_rate))
            return data * mask
            
    def _diagnose_modality(self, z_orig: torch.Tensor, z_aug: torch.Tensor) -> float:
        """Calculate generalization gap for a single modality using silhouette score."""
    ...
    def run(self, original_data_list: list) -> list:
        """
        Execute a complete diagnosis and re-learning flow.

        Args:
            original_data_list (list): List containing original data tensors for each modality.

        Returns:
            list: Alpha values computed for each modality.
        """
        self.encoders.eval()
        gaps = []
...
class Dcpbst(nn.Module):
    def __init__(self, features, sparse=False,
        ...
        self.features = [torch.Tensor(i) for i in features]  # Keep on CPU
        self.sparse = sparse
        self.n_clusters = n_clusters
        ...
        if "ground_truth" in self.adata.obs:
            print("'ground_truth' found, preparing labels for supervised learning...")
            # Convert string labels to integer IDs
            # pandas.factorize returns an integer encoding and a unique label list
            labels_encoded, unique_labels = pd.factorize(self.adata.obs["ground_truth"])
            
            # Convert encoded labels to Tensor and store in model
            self.y_true = torch.tensor(labels_encoded, dtype=torch.long).to(self.device)
            # Define supervised learning prototypes only when labels exist
            self.proxies = nn.Parameter(torch.empty([self.n_clusters, latent_dim * 2]))
            nn.init.xavier_uniform_(self.proxies, gain=1.0)
        else:
            print("Warning: 'Ground Truth' not found in adata.obs. Model can only run in unsupervised mode.")

        # Unsupervised clustering centers defined only when needed
        if self.n_clusters is not None:  # If labels exist, we can also prepare for unsupervised mode
        ...
        self.embed_dim = 512  # Can be adjusted as needed
        self.num_heads = 8   # Number of attention heads
        self.projection_layers = nn.ModuleList([
            nn.Linear(latent_dim, self.embed_dim) for i in range(self.num_views)
        ])
        # Create a projection layer for Katz features as well
        ...
        self.gcn_for_x = GCNModel(
            nfeat=self.embed_dim,  # <-- Use correct dimension variable
            nhid=128,               # e.g., hidden layer dimension
            nclass=self.output_dim,             # e.g., output layer dimension
            dropout=0.5
        )
        self.gcn_for_s = GCNModel(
            nfeat=self.adata.X.shape[1], 
            nhid=128, 
            nclass=self.output_dim, 
            dropout=0.5
        )
        self.read = AvgReadout()  # Global summary
        ...
        self.spatial_s = torch.from_numpy(self.adata.X).float().to(self.device)
        self.spatial_s.requires_grad = True  # Enable backpropagation
        ...
        self.diagnoser = ModalRobustnessDiagnoser(
            ...
            data_types=['gene', 'image'],  # Adjust according to your feature order
            device=self.device,
            n_clusters=self.n_clusters if self.n_clusters is not None else 10  # Provide a default value
        )
        
        # Used to record alpha value change history during modality balance modulation
        # Format: [{'epoch': epoch, 'alphas': [alpha_0, alpha_1, ...]}, ...]
        self.alpha_history = []
    ...
    def encoder_proxies(self):
            """Extract mean and std from shared prototype parameters"""
            mu_proxy = self.proxies[:, :self.latent_dim]
            # Use softplus to ensure sigma is positive and numerically stable
    ...
    def compute_supervised_proxy_loss(self, z, y_true, temperature=0.07):
        """Compute supervised proxy loss between latent feature z and shared prototypes for a single view"""
        mu_proxy, sigma_proxy = self.encoder_proxies()
        
        # Sample prototype centers
        eps_proxy = torch.randn_like(mu_proxy).unsqueeze(1).repeat(1, 10, 1)  # small number of samples
        ...
        # Compute cosine similarity between sample z and all prototypes as attention
        att = F.cosine_similarity(z.unsqueeze(1), z_proxy.unsqueeze(0), dim=-1) # (batch_size, num_classes)
        
        # Create positive and negative example masks
        ...
        logits = att / temperature  # Attention scaling
        loss = F.cross_entropy(logits, y_true)
        return proxy_loss+loss

    def _initialize_cluster_centers(self, features_np):
        """Initialize unsupervised clustering centers using KMeans"""
    ...
    def compute_unsupervised_clustering_loss(self, features):
        """
        Clustering loss computation for unlabeled datasets.
        Assigns each point to the nearest cluster center and minimizes that distance.
        """
        if not self.cluster_centers_initialized:
            self._initialize_cluster_centers(features.detach().cpu().numpy())

        # Compute squared Euclidean distance from each point to all centers
        ...
        # Loss is the mean of all these minimum distances
        clustering_loss = torch.mean(min_dist)

        return clustering_loss

    def update_cluster_centers(self, features, min_dist_indices):
        """Update cluster centers based on minimum distance"""
        # Can be improved with batch update approach
        for i in range(self.n_clusters):
            # Get all samples belonging to this cluster
    ...
    def forward(self,y_true=None):
        ...
        proxy_loss = torch.tensor(0.0, device=self.device)  # Supervised proxy
        clustering_loss = torch.tensor(0.0, device=self.device)  # Unsupervised clustering
        ...
        for i in range(self.num_views):
            ...
            view_emb.append(z)
            recon_loss += F.mse_loss(x_hat, self.pcs[i])  # F.mse_loss is more standard
        ...
        # Average the proxy loss if computed
        if y_true is not None and self.num_views > 0:
            proxy_loss /= self.num_views


        katz = torch.from_numpy(self.katz).float().to(self.device)  # Convert to tensor, spatial features

        final_feature = view_emb +[katz]

        projected_features = []  # Align features
        for i in range(self.num_views):
        ...
        global_feature = torch.mean(stacked_features, dim=1, keepdim=True) # Shape: [N, 1, D] compute average feature as global feature for alignment 
        ...
        self.adj_x = self.adj_x + self.adj_x.T  # Feature neighbor graph
        ...
        # 4. Create PyTorch sparse tensor and move to correct device
        ...
        x_emb=self.gcn_for_x(final_embedding,self.adj_x)  # GCN uses shared encoder and decoder
        s_emb=self.gcn_for_s(self.spatial_s,self.adj_s)
        s_a_emb=self.gcn_for_s(self.spatial_s_a,self.adj_s)

        info_loss=self.info_nce (query=x_emb, positive_key=s_emb, negative_keys=None,temperature=0.05)  # All samples except the corresponding view are negative samples
        ...
        return {
            ...
            "proxy_loss": proxy_loss,  # If y_true is None, this value is 0
            "clustering_loss": clustering_loss,  # If y_true is not None, this value is 0
            "dgi_loss": dgi_loss, 
            "final_embedding": final_embedding}  # Fuse final features and spatial info as clustering feature vector for downstream tasks
    ...
    def fit(self,epochs=900,lr=1e-3, w_cls=10.0,w_recon=10.0, w_kl=0.1, w_pro=2.0,w_info=1.0, w_dgi=0.1,w_clu=2.0,diagnose_every_n_epochs=40):
        ...
        for epoch in tqdm(range(epochs), desc='Training Dcpbst Model'):
            ...
            # Perform forward pass to get dictionary with all losses
                loss_dict = self(y_true=self.y_true)
                logits = loss_dict["logits"]
                # Extract individual losses from dictionary
                ...
                if (epoch + 1) % diagnose_every_n_epochs == 0 and epoch > 0:
                    ...
                    # Record alpha value history for later heatmap plotting
                    self.alpha_history.append({
                        'epoch': epoch + 1,
                        'alphas': alphas.copy()  # Save copy of alpha values
                    })
                    
                    # Print logs to monitor alpha value changes
        ...
        # fit method is typically for training the model itself, can return self
        self.eval()
        with torch.no_grad():
            if self.y_true is not None:
        ...
        embedding_for_clustering = final_embedding.cpu().numpy() # Under no_grad(), detach() is not necessary
        return embedding_for_clustering
    
    def get_alpha_history_for_heatmap(self):
        """
        Get alpha value history, convert to format suitable for heatmap plotting.
        
        Returns:
            dict: Dictionary containing the following keys:
                - 'epochs': list, epoch list
                - 'alpha_matrix': numpy.ndarray, matrix with shape (num_epochs, num_views)
                - 'view_names': list, view name list (e.g. ['View 0', 'View 1'])
        """
        if not self.alpha_history:
            print("Warning: No alpha value history recorded. Ensure diagnostics were run during training.")
            return {
                'epochs': [],
