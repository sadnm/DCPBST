...
**DCPBST** is a deep learning framework for spatial domain identification in spatial transcriptomics. It integrates **gene expression** (scRNA-seq), **histopathological image features** (H&E), and **spatial coordinates** through a unified architecture. DCPBST couples a variational autoencoder with a prototype loss, graph convolutional networks, contrastive learning, and cross-modal attention, which together perform information purification and balanced learning across modalities, thereby fully capturing the contextual information of each node's microenvironment. The purified representations support accurate downstream analysis, and DCPBST outperforms state-of-the-art methods on multiple spatial transcriptomics datasets in both qualitative and quantitative evaluation of spatial domains.

This repository provides the **official reproducibility package** containing all model code and downstream analysis scripts needed to reproduce the main experimental results from the manuscript. Each notebook is self-contained: model hyperparameters are passed directly in code.
...
│   ├── model_827_copy.py            # Main DCPBST model (DLPFC, BRCA, PDAC & Mouse AREI)
...
> **Note**: `data/` and `checkpoints/` are **not** included in this repository — download them from Hugging Face as described above and place them in the repo root. `saved_results/` (trained weights, clustered h5ad, metrics) and `downstream_analysis/deg_results/` (DEG/GO tables and figures) are **generated by running the notebooks / downstream scripts** and are also not shipped.
...
> **Adapting to new datasets**: because data distributions differ across datasets, users applying DCPBST to their own data are encouraged to select and tune an appropriate hyperparameter combination (latent/embedding dimensions, number of attention heads, graph neighbors, loss weights, etc.).
...
The `webtool_*.txt` files of either dataset can be uploaded to the web platform
[https://bioinformatics.com.cn/](https://bioinformatics.com.cn/). All figures
and tables presented in the manuscript were generated using this online
platform. Pre-rendered BRCA GO/KEGG figures are kept in
`figures/BRCA_downanalysis/`.

---

## Citation

The manuscript describing DCPBST is currently under review and is therefore not yet citable. If you
use this code, data, or weights, please cite the manuscript once it is published, and contact the
authors beforehand.
...
All five datasets re-analyzed in this study were curated and released by external sources. Their
unprocessed raw versions can be obtained directly from the original providers:

| Dataset | Original source |
|---------|-----------------|
| **DLPFC** | 10x Visium sections generated by spatialLIBD, accessed through the DeepST repository at [github.com/JiangBioLab/DeepST](https://github.com/JiangBioLab/DeepST) |
| **Mouse anterior brain** | 10x Visium sample `V1_Mouse_Brain_Sagittal_Anterior`, from the 10x Genomics public dataset portal at [10xgenomics.com/datasets](https://www.10xgenomics.com/datasets) |
| **BRCA** | 10x Visium sample `V1_Breast_Cancer_Block_A_Section_1` from the same portal; manual pathological annotations from the SEDR repository at [github.com/JinmiaoChenLab/SEDR](https://github.com/JinmiaoChenLab/SEDR) |
| **PDAC** | Gene Expression Omnibus accession [GSE111672](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE111672); annotations from [cancersrt.info/Download](http://cancersrt.info/Download) |
| **Mouse hypothalamic preoptic** | MERFISH data obtained from SDMBench at [sdmbench.drai.cn](http://sdmbench.drai.cn/) |

The datasets originate from the following publications:
- Maynard, K.R., Collado-Torres, L., Weber, L.M. et al. (2021), *Transcriptome-scale spatial gene expression in the human dorsolateral prefrontal cortex*. **Nature Neuroscience** 24, 425–436 ([10.1038/s41593-020-00787-0](https://doi.org/10.1038/s41593-020-00787-0)) — DLPFC
- Moncada, R., Barkley, D., Wagner, F. et al. (2020), *Integrating microarray-based spatial transcriptomics and single-cell RNA-seq reveals tissue architecture in pancreatic ductal adenocarcinomas*. **Nature Biotechnology** 38, 333–342 ([10.1038/s41587-019-0392-8](https://doi.org/10.1038/s41587-019-0392-8)) — PDAC
- Moffitt, J.R., Bambah-Mukku, D., Eichhorn, S.W. et al. (2018), *Molecular, spatial, and functional single-cell profiling of the hypothalamic preoptic region*. **Science** 362, eaau5324 ([10.1126/science.aau5324](https://doi.org/10.1126/science.aau5324)) — MERFISH mouse hypothalamic preoptic

The preprocessed versions of all five datasets used in this study are deposited at
[huggingface.co/datasets/lanyu1/dcpbst-data](https://huggingface.co/datasets/lanyu1/dcpbst-data)
under DOI [10.57967/hf/9765](https://doi.org/10.57967/hf/9765).
