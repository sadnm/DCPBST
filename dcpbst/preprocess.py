...
def Cal_Feature_Net_Corrected(features_np, n_neighbors=6, metric='euclidean'):
    """
    Build a symmetric K-nearest neighbor graph based on feature matrix.

    Args:
        features_np (np.ndarray): Feature matrix with shape (n_samples, n_features).
        n_neighbors (int): Number of neighbors to keep in adjacency graph.
        metric (str): Distance metric for computation.
...
def Get_feature(adata, emb, deconvolution=False):
    ...
    feat_a = np.random.permutation(emb)
...
def concat_adj(adj_1,adj_2):
    n1, m1 = adj_1.shape
    n2, m2 = adj_2.shape
    adj_1_right = sp.hstack([adj_1, sp.csr_matrix((n1, m2))])
    adj_2_left = sp.hstack([sp.csr_matrix((n2, m1)), adj_2])

    adj_combined = sp.vstack([adj_1_right, adj_2_left])
    return adj_combined
...
class Encoder_cons(Module):
    ...
    def info_nce_loss(self, p, p1, p2, temp=0.2):
        loss_1 = self.info_nce(p, p1, p2, temperature=temp)
        loss_2 = self.info_nce(p1, p, p2, temperature=temp)
